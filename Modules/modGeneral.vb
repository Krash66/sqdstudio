...
#Region "Function Templates for Procedures"
    ...
    Function GetScriptForLOOK(ByVal Task As clsTask) As String
        ...
        Try
            ...
            Dim AutoGen As Boolean = False
            If MsgBox("Would you like Design Studio to create the Variables used for this Template?", MsgBoxStyle.YesNo, "Auto-generate Variables?") = MsgBoxResult.Yes Then
                Prefix = "  "
                AutoGen = True
            End If

            '/// Start Building Template Syntax
            sb.AppendLine("-- replace <search key> with a field in the Source table")
            sb.AppendLine("-- Use Variables created as Source mappings in")
            sb.AppendLine("-- the Mapping Procedure for these fields")
            sb.AppendLine("--")
            sb.AppendLine("LOOK(" & DSlu.DatastoreName & ",'<search key>')")
            sb.AppendLine("IF LOOKFOUND(" & DSlu.DatastoreName & ") = TRUE")
            sb.AppendLine("DO")
            Dim count As Integer = 1
            For Each fld As clsField In LUsel.DSSelectionFields
                Dim varstr As String = "V_LU_" & count.ToString & "_" & fld.FieldName
                sb.AppendLine(TAB & "   " & Prefix & varstr & _
                              " = LOOKFLD(" & DSlu.DatastoreName & "," & fld.FieldName & ")")
                '/// If Variables are to be autoGenerated
                If AutoGen = True Then
                    Dim newvar As clsVariable
                    newvar = New clsVariable(varstr, enumVariableType.VTYPE_GLOBAL)
                    With newvar
                        .Parent = Task.Engine
                        .VarSize = 50
                        .VarInitVal = "' '"
                    End With
                    If newvar.AddNew() = True Then
                        '// add the new variable object to the tree as a "variable" node
                        Dim cNode As New TreeNode
                        cNode = Task.ObjTreeNode   '<-- the Task's treenode
                        cNode = cNode.Parent    '<-- cNode becomes the Task Node's Parent Node the Engine?
                        If CType(cNode.Tag, INode).IsFolderNode = True Then   'If this is the Procs" Folder
                            cNode = cNode.Parent   '<--- Now it's definitely the Engine
                        End If
                        cNode = cNode.Nodes(2)  '<-- Variables folder
                        '/// Add the node as child of "Variables" folder
                        cNode = AddNode(cNode, NODE_VARIABLE, newvar)  '<-- cNode becomes the Variable Node
                        newvar.SeqNo = cNode.Index '//index it's store position
                        AddToCollection(Task.Engine.Variables, newvar)
                    End If
                End If
                count += 1
            Next
